...
def get_marty():
    try:
        marty = Marty("wifi", "192.168.0.102", blocking=True)  # replace with the appropriate address for your Marty
        return marty
...
def perform_action(action, marty):
    ...
    if action == "avancer":
        marty.walk(num_steps=7)
    elif action == "reculer":
        marty.walk(num_steps=7, step_length=-25)
